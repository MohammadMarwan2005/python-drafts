...
class FamilyTree(KnowledgeEngine):
    ...
    # Nice teaching point: this rule fires on a fact ("sister")
    # that another rule INFERRED — not on raw input.
    @Rule(
        Relationship(type="sister", person=MATCH.a, of=MATCH.p),
        Parent(child=MATCH.c, father=MATCH.p),
    )
    def aunt_via_father(self, a, c):
        self.declare(Relationship(type="aunt", person=a, of=c))

    @Rule(
        Relationship(type="sister", person=MATCH.a, of=MATCH.p),
        Parent(child=MATCH.c, mother=MATCH.p),
    )
    def aunt_via_mother(self, a, c):
        self.declare(Relationship(type="aunt", person=a, of=c))
...
engine.declare(Gender(name="Hassan", gender="male"))
engine.declare(Gender(name="Amina", gender="female"))
engine.declare(Parent(child="Marwan", father="Hassan", mother="Amina"))
...
# --- Mariam: Sawsan's sister  =>  aunt of the kids ---
engine.declare(Gender(name="Mariam", gender="female"))
engine.declare(Parent(child="Mariam", father="AbdulKareem", mother="Alia"))
